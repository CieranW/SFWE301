...
|---|---|---|---:|---:|---:|---:|---:|---:|---|
| T1 | Missing authentication and authorization exposes all student data and advisor functionality | Spoofing, Information Disclosure, Elevation of Privilege | 10 | 10 | 10 | 10 | 10 | 10.0 | Critical |
| T2 | Wildcard CORS allows any origin to read API responses from a victim browser | Information Disclosure | 9 | 10 | 9 | 9 | 10 | 9.4 | High |
| T3 | Backend stores and mutates authoritative data in plain CSV files without protection | Tampering, Information Disclosure | 8 | 8 | 7 | 8 | 8 | 7.8 | High |
| T4 | Malformed CSV values can crash startup scoring and break availability | Denial of Service, Tampering | 7 | 9 | 7 | 8 | 7 | 7.6 | High |
| T5 | No transport security if deployed beyond localhost due hardcoded `http://` API usage | Information Disclosure, Tampering | 8 | 8 | 6 | 7 | 9 | 7.6 | High |
| T6 | Role spoofing and insecure direct object access in the UI let users inspect other student records | Spoofing, Information Disclosure, Elevation of Privilege | 7 | 10 | 9 | 7 | 9 | 8.4 | High |
| T7 | No audit logging or security monitoring limits detection and repudiation handling | Repudiation | 6 | 10 | 8 | 8 | 8 | 8.0 | High |
| T8 | [Added] Secrets committed to the repository could disclose sensitive credentials or configuration | Information Disclosure | 10 | 9 | 7 | 10 | 9 | 9.0 | High |
| T9 | [Added] CSV formula injection could execute spreadsheet formulas when exported or opened in Excel-like tools | Tampering, Elevation of Privilege | 8 | 9 | 8 | 8 | 8 | 8.2 | High |
| T10 | [Added] Untrusted CSV-backed content rendered in the frontend could create XSS risk if unsafe rendering is introduced later | Tampering, Information Disclosure | 6 | 6 | 5 | 6 | 7 | 6.0 | Medium |
| T11 | [Added] Debug errors or verbose stack traces could leak implementation details and data paths | Information Disclosure | 6 | 9 | 9 | 6 | 9 | 7.8 | High |
| T12 | [Added] Oversized CSV files can exhaust memory or degrade backend availability during full-file reads and startup scoring | Denial of Service | 7 | 8 | 8 | 9 | 7 | 7.8 | High |
...
|---|---|---|---:|---:|---:|---:|---:|---:|---|
| 1 | Broken Access Control on API | S / E / I | 10 | 10 | 10 | 10 | 10 | 10.0 | Critical |
| 2 | Wildcard CORS Exposure | I | 9 | 10 | 9 | 9 | 10 | 9.4 | High |
| 3 | Unauthorized Student Data Exposure | I / E | 10 | 10 | 10 | 10 | 10 | 10.0 | Critical |
| 4 | CSV Data Tampering | T | 8 | 8 | 7 | 8 | 8 | 7.8 | High |
| 5 | Malformed CSV Causes Startup Failure | D / T | 7 | 9 | 7 | 8 | 7 | 7.6 | High |
| 6 | DoS via Large CSV File | D | 7 | 8 | 8 | 9 | 7 | 7.8 | High |
| 7 | Secrets in Repository | I | 10 | 9 | 7 | 10 | 9 | 9.0 | High |
| 8 | Malicious CSV Injection (Formula Injection) | T / E | 8 | 9 | 8 | 8 | 8 | 8.2 | High |
| 9 | XSS via CSV Content | T / I | 6 | 6 | 5 | 6 | 7 | 6.0 | Medium |
| 10 | Information Disclosure (Debug Errors / Stack Traces) | I | 6 | 9 | 9 | 6 | 9 | 7.8 | High |
| 11 | No Transport Security Beyond Localhost | I / T | 8 | 8 | 6 | 7 | 9 | 7.6 | High |
| 12 | Lack of Audit Logging / Monitoring | R | 6 | 10 | 8 | 8 | 8 | 8.0 | High |
...
|---|---|---:|---:|---:|---:|---:|---:|---|---|
| Spring Boot API | Broken access control allows any user to call student and advisor data endpoints | 10 | 10 | 10 | 10 | 10 | 50 | Critical | Add backend authentication and role-based authorization; expose student-specific endpoints such as `/me` instead of full datasets. |
| Spring Boot API | Wildcard CORS allows any origin to read API responses from a victim browser | 9 | 10 | 9 | 9 | 10 | 47 | High | Restrict CORS to trusted frontend origins and pair it with server-side auth. |
| Student records in `backend/student.csv` | Unauthorized API access reveals names, GPA, major, year, and match information | 10 | 10 | 10 | 10 | 10 | 50 | Critical | Limit API responses by role, stop returning all student records to the browser, and protect the backing data source. |
| Student records in `backend/student.csv` | CSV tampering could alter student details or recommendation scores | 8 | 8 | 7 | 8 | 8 | 39 | High | Restrict file permissions, validate imported data, and move authoritative records to a database with access control. |
| Recommendation scoring process | Malformed GPA or scholarship values can crash scoring during startup | 7 | 9 | 7 | 8 | 7 | 38 | High | Add validation, fail safely per record, and move scoring logic out of `main` into a tested service layer. |
| Recommendation scoring process | Oversized CSV files can increase startup time and memory usage | 7 | 8 | 8 | 9 | 7 | 39 | High | Stream file processing, add size limits, and avoid `Files.readAllLines` for large inputs. |
| Git repository / source history | Secrets accidentally committed to the repo could expose credentials or sensitive data | 10 | 9 | 7 | 10 | 9 | 45 | High | Run secret scanning, use environment variables or a secret manager, and rotate any exposed secrets. |
| Scholarship and student CSV content | Malicious formula injection could execute when opened in spreadsheet software | 8 | 9 | 8 | 8 | 8 | 41 | High | Escape or neutralize fields beginning with `=`, `+`, `-`, or `@` before export or spreadsheet use. |
| React frontend | CSV-backed content could become an XSS vector if unsafe HTML rendering is introduced later | 6 | 6 | 5 | 6 | 7 | 30 | Medium | Keep React escaping in place, avoid `dangerouslySetInnerHTML`, and sanitize future rich text fields. |
| Spring Boot API | Debug errors or stack traces could expose internal file paths and implementation details | 6 | 9 | 9 | 6 | 9 | 39 | High | Add centralized exception handling and return generic production error responses. |
| Frontend to backend transport | Hardcoded `http://localhost:8080` usage is unsafe if reused outside local development | 8 | 8 | 6 | 7 | 9 | 38 | High | Use environment-based API URLs, relative paths where appropriate, and enforce HTTPS in deployed environments. |
| Spring Boot API and admin workflows | Missing audit logs make abusive access and data changes hard to detect or prove | 6 | 10 | 8 | 8 | 8 | 40 | High | Add structured access logs, admin action logs, and alerting for suspicious activity. |
